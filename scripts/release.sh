...
#
# Reads the version from crates/bitloops-platform-embeddings/Cargo.toml on main
# (must match src/bitloops_local_embeddings/version.py), creates tag vX.Y.Z,
# and pushes the tag only.
# Version bump must already be merged via PR.

CARGO="crates/bitloops-platform-embeddings/Cargo.toml"
VERSION_PY="src/bitloops_local_embeddings/version.py"
...
PY_BARE=$(grep '^__version__' "$VERSION_PY" | head -1 | sed 's/__version__ = "\(.*\)"/\1/')
if [[ -z "$PY_BARE" ]]; then
  echo "Error: could not read __version__ from $VERSION_PY"
  exit 1
fi
if [[ "$BARE" != "$PY_BARE" ]]; then
  echo "Error: version mismatch: $CARGO has $BARE but $VERSION_PY has $PY_BARE"
  exit 1
fi
