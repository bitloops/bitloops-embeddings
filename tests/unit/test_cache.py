...
from bitloops_local_embeddings.cache import CACHE_ENV_VAR, resolve_cache_dir
...
def test_platform_default_cache_dir_is_used_as_fallback(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(
        "bitloops_local_embeddings.cache.user_cache_dir",
        lambda app_name: str(tmp_path / app_name),
    )

    assert resolve_cache_dir() == tmp_path / "bitloops-local-embeddings"
